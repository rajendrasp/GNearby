...
void NearbyConnectionImpl::Read(ReadCallback callback) {
  MutexLock lock(&mutex_);
  if (reads_.empty()) {
      NL_LOG(INFO) << "LOGINFO read is attempted but bytes are not there so storing the callback";
    read_callback_ = std::move(callback);
    return;
  }

  NL_LOG(INFO) << "LOGINFO read is attempted and bytes available so invoking callback";
...
void NearbyConnectionImpl::WriteMessage(std::vector<uint8_t> bytes) {
  MutexLock lock(&mutex_);
  if (read_callback_) {
      NL_LOG(INFO) << "LOGINFO read_callback_ is found so invoking callback";
  ...
  NL_LOG(INFO) << "LOGINFO read_callback_ is NOT found so storing bytes for later read";
