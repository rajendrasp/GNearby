...
void PairedKeyVerificationRunner::SendPairedKeyResultFrame(
  ...
  result_frame->set_os_type(ToProtoOsType(hardcoded::GetDeviceInfoOsType()));
...
void PairedKeyVerificationRunner::SendPairedKeyEncryptionFrame()
{
	...
	if (!signature.has_value() || signature->empty())
	{
		signature = hardcoded::GenerateRandomBytes(kNearbyShareNumBytesRandomSignature);
	}
	...
	{
		certificate_id_hash =
			hardcoded::GenerateRandomBytes(kNearbyShareNumBytesAuthenticationTokenHash);
	}
...
PairedKeyVerificationRunner::VerifyRemotePublicCertificate(
  ...
  return VerifyRemotePublicCertificateWithPrivateCertificate(
      DeviceVisibility::DEVICE_VISIBILITY_EVERYONE, frame);
}
